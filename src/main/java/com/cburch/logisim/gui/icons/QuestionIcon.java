...
public class QuestionIcon implements Icon {
  ...
  public void paintIcon(Component c, Graphics g, int x, int y) {
    ...
    final java.awt.Font f = g2.getFont().deriveFont((float) width / (float) 1.3).deriveFont(Font.BOLD);
    final java.awt.font.TextLayout t = new TextLayout("?", f, g2.getFontRenderContext());
    final float centerX = (float) width / (float) 2 - (float) t.getBounds().getCenterX();
    final float centerY = (float) width / (float) 2 - (float) t.getBounds().getCenterY();

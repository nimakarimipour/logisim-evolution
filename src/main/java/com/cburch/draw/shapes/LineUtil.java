...
public final class LineUtil {
  ...
  private static double[] nearestPoint(
    ...
    final double num = (xq - x0) * dx + (yq - y0) * dy;
    double u;
    if (isSegment) {
      if (num < 0) u = 0;
  ...
  public static double ptDistSqSegment(
    ...
    final double u = ((xq - x0) * dx + (yq - y0) * dy) / len2;
  ...
  public static Location snapTo8Cardinals(Location from, int mx, int my) {
    ...
    if (mx != px && my != py) {
      final double ang = Math.atan2(my - py, mx - px);
      final int d45 = (Math.abs(mx - px) + Math.abs(my - py)) / 2;
      final int d = (int) (4 * ang / Math.PI + 4.5);
      switch (d) {
        case 0:
        case 8: // going west
        case 4: // going east
